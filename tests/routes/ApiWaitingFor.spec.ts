...
describe('ApiWaitingFor', function() {
  ...
  it('fails when game not found', async () => {
    scaffolding.url = '/terraforming/api/waitingfor?id=p-some-player-id&gameAge=123&undoCount=0';
  ...
  it('fails when player not found', async () => {
    ...
    scaffolding.url = '/terraforming/api/waitingfor?id=' + player.id + '&gameAge=50&undoCount=0';
    await scaffolding.get(ApiWaitingFor.INSTANCE, res);
    expect(res.statusCode).eq(404);
  ...
  it('fails when spectator not found', async () => {
    ...
    scaffolding.url = '/terraforming/api/waitingfor?id=' + game.spectatorId + '-invalid' + '&gameAge=50&undoCount=0';
    await scaffolding.get(ApiWaitingFor.INSTANCE, res);
    expect(res.statusCode).eq(404);
    expect(res.content).eq('Not found: cannot find game for that player');
  });
  ...
  it('sends model for spectator', async () => {
    ...
    scaffolding.url = '/terraforming/api/waitingfor?id=' + game.spectatorId + '&gameAge=50&undoCount=0';
    await scaffolding.get(ApiWaitingFor.INSTANCE, res);
    expect(res.statusCode).eq(200);
